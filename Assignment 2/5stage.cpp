...
struct MIPS_Architecture
{
	...
	*/
	...
	void fivestage_piplined_no_bypass()
	{
	...
	if(!latch3.command.empty())
	{
		...
		{
			int adr=sw(latch3.command[1],latch3.command[2],latch3.command[3]);
		...
		if(latch3.command[0]=="add" || latch3.command[0]=="sub" || latch3.command[0]=="mul" || latch3.command[0]=="slt" || latch3.command[0]=="addi")
		{
			int result=instructions[latch3.command[0]](*this, latch3.command[1], latch3.command[2], latch3.command[3]);
		...
		{
			instructions[latch3.command[0]](*this, latch3.command[1], latch3.command[2], latch3.command[3]);
			branch_stall=false;
...
int main(int argc, char *argv[])
{
	...
	else
	{
	 ...
	 mips->fivestage_piplined_no_bypass();
	return 0;
}